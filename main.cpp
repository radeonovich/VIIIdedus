...
struct ls;
typedef ls * link;
typedef enum{Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec,UNK} item; // создается перечисление месяцев и называется item
char month[12][4]={"Jan","Feb","Mar","Apr","May","Jun","Jul","Aug",
                   "Sep","Oct","Nov","Dec"};
struct ls { // структура с параметрами body (значение), предыдщуий и следующий
    item body;
    link pre, next;
}
*list, *t,*f; // что это?
